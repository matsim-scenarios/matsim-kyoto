 ...
 * <p>
 * Experimental!
 * Note: This class was copied from pt2matsim to make same modifications!
 *
 * @author polettif
 * @author vsp
 */
@SuppressWarnings({"CyclomaticComplexity", "JavaNCSS", "UnnecessaryParenthesesExtended", "MultipleVariableDeclarations",
	"DesignForExtension"})
public class OsmTransitScheduleConverter {
	...
	/**
	 * Converts relations, nodes and ways from osm to an unmapped MATSim Transit Schedule.
	 */
	public void convert(TransitSchedule schedule, CoordinateTransformation transformation) {
		...
		for (Osm.Relation relation : osmData.getRelations().values()) {
			...
			if (route_master.matches(relation)) {
				...
				String name = null;
				if (relation.getTags().containsKey("name:en")) {
					name = relation.getValue("name:en");
				} else if (relation.getTags().containsKey("name")) {
					name = relation.getValue("name");
				}

				newTransitLine.setName(name);
	...
	protected TransitStopFacility createStopFacilityFromOsmNode(Osm.Node node, String stopAreaIdString) {
		...
		String name = null;
		if (node.getTags().containsKey("name:en")) {
			name = node.getValue("name:en");
		} else if (node.getTags().containsKey("name")) {
			name = node.getValue("name");
		}

		newStopFacility.setName(name);
	...
	protected String createStringId(Osm.Relation relation) {
		String id;
		String name = null;
		boolean ref = false, operator = false;
